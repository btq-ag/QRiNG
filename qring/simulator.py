...
class QRiNGSimulator:
    ...
    def __init__(
        self,
        numNodes: int = 6,
        bitstringLength: int = 8,
        seed: int | None = None,
        useQuantumBackend: bool | None = None,
        consensusThreshold: int | None = None,
    ) -> None:
        """
        ...
        if useQuantumBackend is True and not _HAS_QISKIT:
            raise RuntimeError(
                "qiskit-aer is required for the quantum backend. "
        ...
        self.consensusThreshold = (
            consensusThreshold if consensusThreshold is not None else bitstringLength // 2
        )
        self.nodes = list(range(numNodes))
        self.bitstrings: dict[int, npt.NDArray[np.int_]] = {}
        self.voteCounts = np.zeros(numNodes)
        self.hasVoted = np.zeros(numNodes, dtype=bool)
        self.honestNodes: list[int] = []
        self.finalRandomBits: npt.NDArray[np.int_] | None = None
    ...
    def calculateBitstringSimilarity(self, node1: int, node2: int) -> int:
        ...
        matches: int = int(np.sum(self.bitstrings[node1] == self.bitstrings[node2]))
        return matches
...
def createQringNetworkVisualization(simulator: QRiNGSimulator, savePath: str) -> None:
    ...
    bars = ax3.bar(
        [f"N{i}" for i in simulator.nodes],
        simulator.voteCounts,
        color=node_colors,
        alpha=0.7,
        edgecolor="black",
    )
    ...
    ax4.imshow(bitstring_matrix, cmap="RdYlBu", aspect="auto")
    ...
    for i, (x, y, text) in enumerate(steps):
        ...
        if i < len(steps) - 1:
            ax5.arrow(x + 0.5, y, 1, 0, head_width=0.1, head_length=0.2, fc="black", ec="black")
    ...
    ax5.text(1, 1.5, r"$s_{i,j} = \{m(n) | b_i(n) = b_j(n)\}_n$", fontsize=10, ha="center")
    # Matching count formula from smart contract
    ax5.text(
        3,
        1.5,
        r"$match(i,j) = \sum_{n=1}^l \mathbb{I}(s_{i,j}(n) = s_{i,i}(n))$",
        fontsize=10,
        ha="center",
    )
    # Honest node threshold condition
    ax5.text(5, 1.5, r"$honest \Leftarrow match(i,j) > \frac{l}{2}$", fontsize=10, ha="center")
    # Final random number generation formula
    ax5.text(7, 1.5, r"$R(n) = \bigoplus_{v_i \in V_{honest}} s_i(n)$", fontsize=10, ha="center")
...
def createQuantumMeasurementVisualization(
    simulator: QRiNGSimulator, savePath: str, seed: int | None = None
) -> None:
    """
    ...
    ax1.scatter(
        correlation_matrix[:, 0] + rng.normal(0, 0.05, len(correlation_matrix)),
        correlation_matrix[:, 1] + rng.normal(0, 0.05, len(correlation_matrix)),
        alpha=0.6,
        c=range(len(correlation_matrix)),
        cmap="viridis",
    )
    ...
    ax1.text(
        0.05,
        0.95,
        f"Correlation: {correlation_coeff:.3f}",
    ...
    ax2.text(
        np.pi / 4,
        0.8,
        r"$|\psi\rangle = \cos(\theta/2)|0\rangle + \sin(\theta/2)|1\rangle$",
        fontsize=11,
        bbox=dict(boxstyle="round,pad=0.3", facecolor="yellow", alpha=0.7),
    )
    ...
    ax3.text(
        0.05,
        0.95,
        f"Total Entropy: {entropy:.3f} bits\nMax Entropy: 1.000 bits",
    ...
    if simulator.honestNodes and len(simulator.honestNodes) > 1:
        ...
        colors = plt.cm.Set3(np.linspace(0, 1, len(simulator.honestNodes)))  # type: ignore[attr-defined]
        ...
        for i, node in enumerate(simulator.honestNodes):
            ...
            for j, bit in enumerate(bitstring):
                ...
                ax4.add_patch(rect)
                ax4.text(
                    j + 0.5, y_pos - i - 0.5, str(bit), ha="center", va="center", fontweight="bold"
                )
            ...
            # Add node label
            ax4.text(-0.5, y_pos - i - 0.5, f"N{node}", ha="center", va="center", fontweight="bold")
        ...
        for j, bit in enumerate(cumulative_xor):
            ...
            rect = patches.Rectangle(
                (j, -2), 1, 0.8, linewidth=2, edgecolor="black", facecolor=color, alpha=0.9
            )
            ax4.add_patch(rect)
            ax4.text(
                j + 0.5, -1.5, str(bit), ha="center", va="center", fontweight="bold", fontsize=12
            )
...
def runSimulationAndCreateVisualizations() -> tuple[QRiNGSimulator, dict]:
    ...
    bits = simulator.finalRandomBits
    finalStr = "".join(map(str, bits)) if bits is not None else "None"
    print(f"Final random number: {finalStr}")
