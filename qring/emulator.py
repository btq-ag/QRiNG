...
class QRiNGEmulator:
    ...
    def __init__(
        self,
        bitstringLength: int = 6,
        adminAddress: str | None = None,
        ...
        # Emulation tracking
        self.transactionLog: list[dict] = []
        self.gasConsumption: dict[str, list[int]] = {}
        self.eventsEmitted: list[dict] = []
            ...
            for i in range(len(self.voters)):
                if fromNode != i:
                    ...
                    for j in range(len(self.voters[fromNode]["bitstring"])):
                        if self.voters[fromNode]["bitstring"][j] == self.voters[i]["bitstring"][j]:
                            add += 1
...
def createSmartContractExecutionVisualization(emulator: QRiNGEmulator, save_path: str) -> None:
    ...
    if emulator.voters:
        ...
        for i, voter in enumerate(emulator.voters):
            ...
            row = [voter["voteCount"], 1 if voter["hasVoted"] else 0, voter["number"]]
        ...
        for i in range(len(voter_labels)):
            for j in range(3):
                ax2.text(j, i, f"{state_matrix[i, j]}", ha="center", va="center", fontweight="bold")
            ...
            for j in range(num_voters):
                if i != j:
                    matches = sum(
                        1
                        for a, b in zip(
                            emulator.voters[i]["bitstring"], emulator.voters[j]["bitstring"]
                        )
                        if a == b
                    )
    ...
    functions_timeline: list[tuple[float, float, str]] = [
        (2, 2.5, "addNewString()"),
        (4, 2.5, "setAddresses()"),
        (6, 2.5, "check() × N"),
        (8, 2.5, "endVoting()"),
    ...
    for x_pos, y_pos, func_text in functions_timeline:
        ax5.text(
            x_pos,
            y_pos,
            func_text,
    ...
    if emulator.voters:
        ...
        ax6.imshow(bitstring_matrix, cmap="RdYlBu", aspect="auto")
        ax6.set_yticks(range(num_voters))
        ax6.set_yticklabels(
            [f"V{i} (VC:{emulator.voters[i]['voteCount']})" for i in range(num_voters)]
        )
