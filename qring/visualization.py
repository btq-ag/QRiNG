...
class QRiNGVisualizer:
    ...
    def animateQkdProcess(self, savePath: str) -> None:
        ...
        def animate(frame):
            ...
            for i, (node1, node2) in enumerate(connections):
                if i <= phase // 10:  # Progressive connection establishment
                    ...
                    alpha = (
                        0.7 * (1 + np.sin(phase * 0.5 + i)) / 2
                    )  # Oscillating correlation strength
                    ax1.plot(
                        [x1, x2],
                        [y1, y2],
                    ...
                    entangle_circle = Circle(
                        (mid_x, mid_y), 0.05, facecolor=self.colors["active"], alpha=alpha
                    )
            ...
            for node_id in range(4):
                ...
                amplitude_real = np.cos(time_steps + node_id * np.pi / 4 + phase * 0.1) * np.exp(
                    -time_steps / 15
                )
                amplitude_imag = np.sin(time_steps + node_id * np.pi / 4 + phase * 0.1) * np.exp(
                    -time_steps / 15
                )
            ...
            ax2.axvline(
                x=phase / 6, color="red", linestyle="-", alpha=0.7, label="Measurement Time"
            )
            ...
            if phase > 20:
                ...
                ax4.set_ylim(0, 1)
                ax4.axhline(
                    y=0.5, color="red", linestyle="--", alpha=0.7, label="Perfect Randomness"
                )
                ax4.legend()

            # Add phase indicator
            fig.suptitle(
                f"QRiNG: Quantum Key Distribution Process (Phase: {phase}/60)",
                fontsize=18,
                fontweight="bold",
            )
        ...
        anim = animation.FuncAnimation(
            fig, animate, init_func=init, frames=60, interval=150, blit=False
        )
    ...
    def animateConsensusMechanism(self, savePath: str) -> None:
        ...
        def animate(frame):
            ...
            for node in range(6):
                ...
                if phase > 10:
                    ax1.text(
                        x,
                        y - 0.35,
                        f"Votes: {int(simulator.voteCounts[node])}",
                        ha="center",
                        va="center",
                        fontsize=8,
                    )
                    ...
                    for j in range(6):
                        if i != j:
                            ...
                            similarity_matrix[i, j] = (
                                similarity / simulator.bitstringLength
                            )  # Normalize
            ...
            if phase > 15:
                ...
                bars = ax3.bar(
                    ...
                    voteCounts,
                    color=[
                        self.colors["honest"] if vc > 3 else self.colors["classical"]
                ...
                ax3.axhline(y=3, color="red", linestyle="--", alpha=0.7, label="Honesty Threshold")
                ax3.legend()
            ...
            if phase > 30:
                ...
                wedges, texts, autotexts = ax4.pie(
                    values, labels=labels, colors=colors, autopct="%1.0f", startangle=90
                )
            ...
            # Add phase indicator
            phase_text = (
                f"Phase: Checking Node {current_checker}"
                if phase < 60
                else "Phase: Consensus Complete"
            )
            fig.suptitle(
                f"QRiNG: Consensus Mechanism - {phase_text}", fontsize=18, fontweight="bold"
            )
        ...
        # Create and save animation
        anim = animation.FuncAnimation(
            fig, animate, init_func=init, frames=72, interval=200, blit=False
        )
    ...
    def animateSmartContractExecution(self, savePath: str) -> None:
        ...
        def animate(frame):
            ...
            ax1.text(
                2,
                7,
                f"Admin: {emulator.admin[:10] if emulator.admin else 'None'}...",
                ha="center",
                va="center",
                fontsize=10,
                fontweight="bold",
            ...
            voting_color = (
                self.colors["active"] if emulator.votingActive else self.colors["inactive"]
            )
            voting_box = FancyBboxPatch(
                (4.5, 6.5),
                3,
                1,
                boxstyle="round,pad=0.1",
            ...
            ax1.text(
                6,
                7,
                f"Voting: {'Active' if emulator.votingActive else 'Inactive'}",
                ha="center",
                va="center",
                fontsize=10,
                fontweight="bold",
            ...
            if emulator.voters:
                for i, voter in enumerate(emulator.voters):
                    ...
                    if y_pos > 0:  # Only show if within visible area
                        ...
                        voter_color = (
                            self.colors["honest"] if voter["hasVoted"] else self.colors["inactive"]
                        )
                        voter_box = FancyBboxPatch(
                            (0.5, y_pos - 0.4),
                            8,
                            0.8,
                        ...
                        if "bitstring" in voter:
                            for j, bit in enumerate(voter["bitstring"]):
                                bit_color = (
                                    self.colors["honest"] if bit == 1 else self.colors["classical"]
                                )
                                bit_rect = Rectangle(
                                    (7.5 + j * 0.15, y_pos - 0.1),
                                    0.1,
                                    0.2,
                                    facecolor=bit_color,
                                    edgecolor="black",
                ...
                try:
                    ...
                    if bits is not None:
                        ...
                        for i, bit in enumerate(bits):
                            ...
                            rect = Rectangle(
                                (i, 0), 0.8, 0.8, facecolor=color, edgecolor="black", linewidth=2
                            )
                            ax4.add_patch(rect)
                            ax4.text(
                                i + 0.4,
                        ...
                        decimalValue = sum(
                            bit * (2 ** (len(bits) - 1 - i)) for i, bit in enumerate(bits)
                        )
                        ax4.text(
                            len(bits) / 2,
                            -0.3,
                            f"Decimal: {decimalValue}",
                            ha="center",
                            va="center",
                            fontsize=12,
                            fontweight="bold",
            ...
            fig.suptitle(
                f"QRiNG: Smart Contract Execution - {phase_names[current_phase_idx]}",
                fontsize=18,
                fontweight="bold",
            )
        ...
        # Create and save animation
        anim = animation.FuncAnimation(
            fig, animate, init_func=init, frames=80, interval=150, blit=False
        )
    ...
    def animateProtocolComparison(self, savePath: str) -> None:
        ...
        def animate(frame):
            ...
            emu_honest = len(
                [v for v in emulator.voters if v["voteCount"] > len(emulator.voters) // 2]
            )
            ...
            ax1.bar(
                ...
                width,
                label="Emulator",
                color=self.colors["smart_contract"],
                alpha=0.7,
            )
                ...
                try:
                    ...
                    if sim_bits is not None and cachedEmuBits is not None:
                        ...
                        for i, bit in enumerate(sim_bits):
                            ...
                            rect = Rectangle(
                                (i, 1.5), 0.8, 0.8, facecolor=color, edgecolor="black", linewidth=1
                            )
                            ax3.add_patch(rect)
                            ax3.text(
                                i + 0.4,
                        ...
                        for i, bit in enumerate(emu_bits):
                            color = (
                                self.colors["smart_contract"]
                                if bit == 1
                                else self.colors["inactive"]
                            )
                            rect = Rectangle(
                                (i, 0.5), 0.8, 0.8, facecolor=color, edgecolor="black", linewidth=1
                            )
                            ax3.add_patch(rect)
                            ax3.text(
                                i + 0.4,
                        ...
                        match = "Match" if np.array_equal(sim_bits, emu_bits) else "Mismatch"
                        ax3.set_title(
                            f"Random Bits: {match}",
                            fontsize=14,
                        )
...
• Random Bits Generated: {"Yes" if simulator.finalRandomBits is not None else "No"}
        ...
        # Create and save animation
        anim = animation.FuncAnimation(
            fig, animate, init_func=init, frames=60, interval=200, blit=False
        )
